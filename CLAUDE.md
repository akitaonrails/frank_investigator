...
- **LLM provider is configurable.** The app routes chat analysis through `RubyLLM` using `FRANK_INVESTIGATOR_LLM_PROVIDER` and `FRANK_INVESTIGATOR_LLM_MODELS`. Production currently routes chat through OpenRouter with `x-ai/grok-4.3`, and uses direct OpenAI for embeddings.
...
- **Background jobs via Solid Queue.** Production runs Solid Queue in a dedicated Kamal `worker` role via `bin/jobs start`; the web role does not embed Solid Queue inside Puma. Fetch-heavy jobs use a dedicated low-concurrency `fetch` queue to keep Chromium failures from wedging the rest of the pipeline. UI refresh broadcasts (`Turbo::Streams::BroadcastStreamJob`) run on a dedicated `realtime` queue with its own worker thread so a backlog of stale broadcasts (typical after a deploy pause) can never starve actual investigation work on `default`. Recurring jobs remain defined in `config/recurring.yml`.
...
- **Stale-job cleanup is automated.** `QueueMaintenance::PurgeStaleBroadcasts` (every 5 min) drops Turbo broadcasts that have been ready longer than 5 minutes, and `QueueMaintenance::PurgeStaleRecurringJobs` (every 15 min) drops `solid_queue_recurring` ticks older than 15 minutes. Both target only `ReadyExecution` rows and skip anything already claimed, scheduled, blocked, or failed, so racing the dispatcher is safe. If you change the queue layout, update these services as well.
...
- **Auto-submission of related coverage is a closed loop.** After cross-reference, `Investigations::AutoSubmitRelatedJob` submits up to `FRANK_INVESTIGATOR_AUTO_SUBMIT_MAX` (default 5) related articles found by the coordinated narrative detector. Candidates are deduplicated by host (and the parent's own host is excluded) so each submission is a different editorial voice. Every child is tagged with `Investigation#auto_submitted_from_id` pointing at the parent. When a child completes, `Investigations::GenerateSummaryJob` enqueues `Investigations::RefreshParentEnrichmentJob` for the parent, which pessimistically locks the parent row, re-runs `Analyzers::CrossInvestigationEnricher` (refreshes `event_context`) and `Analyzers::HonestHeadlineGenerator` (regenerates `honest_headline` against the new context), then stamps `last_enrichment_refresh_at`. The timestamp makes the refresh idempotent — concurrent triggers from sibling children no-op unless a strictly newer child has completed.
